...
"""
8-GPU Long-Context Comparison — Best 5 configurations for 6 deployment modes.

Stresses the prefill stage with ISL=32000 (32K context) to analyze how
different deployment modes handle long-context workloads on 8 GPUs.
...
Usage:
    ...
    python tools/plot_8gpu_comparison_long_context.py              # run search + plot
    python tools/plot_8gpu_comparison_long_context.py --from-csv   # reuse saved CSVs (skip search)
"""
...
ISL = 32000       # 32K context — heavy prefill pressure
OSL = 500
PREFIX = 0
TTFT = 5000.0     # 5 s TTFT budget for 32K prefill
...
# Tag used for CSV / plot filenames to distinguish from the short-context run
_CONTEXT_TAG = "longctx"
...
def print_top_k_table(
    ...
    for mode_key, df in mode_dfs.items():
        ...
        top = extract_top_k(df, metric, TOP_K)
        if top.empty:
            print(f"\n  [{mode_key}] — no results")
            continue
...
# CSV I/O
# =========================================================================
CSV_PREFIX = f"plot_8gpu_{_CONTEXT_TAG}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot top-5 configs for 6 deployment modes on 8 GPUs — long context (ISL=32K).",
    )
    parser.add_argument(
...
def main():
    ...
    print(f"8-GPU Long-Context Comparison: {MODEL_PATH} on {SYSTEM}")
    print(f"  ISL={ISL}, OSL={OSL}, TTFT={TTFT}ms, Backend={BACKEND} {VERSION}")
    print(f"  Top-K = {TOP_K}  (long context — heavy prefill pressure)")
    ...
    plot_metric(
        ...
        y_metric="seq/s",
        y_label="seq/s",
        title=f"Top-{TOP_K} Configs by seq/s — {TOTAL_GPUS} GPUs, ISL={ISL} ({MODEL_PATH})",
        filename=f"plot_8gpu_{_CONTEXT_TAG}_top5_seqs_{SYSTEM}.png",
    )
    ...
    plot_metric(
        ...
        y_metric="tokens/s",
        y_label="tokens/s",
        title=f"Top-{TOP_K} Configs by tokens/s — {TOTAL_GPUS} GPUs, ISL={ISL} ({MODEL_PATH})",
        filename=f"plot_8gpu_{_CONTEXT_TAG}_top5_tokenss_{SYSTEM}.png",
    )
    ...
    plot_metric(
        ...
        title=f"Top-{TOP_K} Fastest Configs by E2E Latency — {TOTAL_GPUS} GPUs, ISL={ISL} ({MODEL_PATH})",
        filename=f"plot_8gpu_{_CONTEXT_TAG}_top5_e2e_latency_{SYSTEM}.png",
        ascending=True,
    )
    ...
    plot_metric(
        ...
        y_metric="ttft",
        y_label="TTFT (ms)",
        title=f"Top-{TOP_K} Fastest Configs by TTFT — {TOTAL_GPUS} GPUs, ISL={ISL} ({MODEL_PATH})",
        filename=f"plot_8gpu_{_CONTEXT_TAG}_top5_ttft_{SYSTEM}.png",
        ascending=True,
    )

    print(f"\nDone. Plots saved as:")
    print(f"  plot_8gpu_{_CONTEXT_TAG}_top5_seqs_{SYSTEM}.png")
    print(f"  plot_8gpu_{_CONTEXT_TAG}_top5_tokenss_{SYSTEM}.png")
    print(f"  plot_8gpu_{_CONTEXT_TAG}_top5_e2e_latency_{SYSTEM}.png")
    print(f"  plot_8gpu_{_CONTEXT_TAG}_top5_ttft_{SYSTEM}.png")
