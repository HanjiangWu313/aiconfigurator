...
def _apply_missing_quant_defaults(model_config: config.ModelConfig) -> None:
...
def _apply_model_quant_defaults(
    ...
    _apply_missing_quant_defaults(model_config)
...
class BaseModel:
    ...
    # Substring markers — see _is_attn_op below. Kept as a class attribute so
    # external tooling (e.g. tools/afd_transfer_tracker.py) can introspect it.
    _ATTN_COMPONENTS = {
    ...
    # ------------------------------------------------------------------
    # AFD 4-stage pipeline op-list properties.
    # 2-group properties (context_attn_ops / context_ffn_ops / ...)
    # are defined further below using upstream's substring-based
    # _is_attn_op / _is_ffn_op classifiers.
    # ------------------------------------------------------------------
    def _is_comm_a2f_op(self, op_name: str) -> bool:
        return any(m in op_name for m in self._COMM_A2F_COMPONENTS)

    def _is_comm_f2a_op(self, op_name: str) -> bool:
        return any(m in op_name for m in self._COMM_F2A_COMPONENTS)

    def _afd_4stage_partition(self, ops_list):
        """Split *ops_list* into (attn_compute, comm_a2f, ffn_compute, comm_f2a)."""
        s1, s2, s3, s4 = [], [], [], []
        for op in ops_list:
            n = op._name
            if self._is_comm_a2f_op(n):
                s2.append(op)
            elif self._is_comm_f2a_op(n):
                s4.append(op)
            elif self._is_attn_op(n):
                s1.append(op)
            else:
                s3.append(op)
        return s1, s2, s3, s4
    ...
    @property
    def context_attn_compute_ops(self):
        return self._afd_4stage_partition(self.context_ops)[0]

    @property
    def context_comm_a2f_ops(self):
        return self._afd_4stage_partition(self.context_ops)[1]

    @property
    def context_ffn_compute_ops(self):
        return self._afd_4stage_partition(self.context_ops)[2]

    @property
    def context_comm_f2a_ops(self):
        return self._afd_4stage_partition(self.context_ops)[3]

    @property
    def generation_attn_compute_ops(self):
        return self._afd_4stage_partition(self.generation_ops)[0]

    @property
    def generation_comm_a2f_ops(self):
        return self._afd_4stage_partition(self.generation_ops)[1]

    @property
    def generation_ffn_compute_ops(self):
        return self._afd_4stage_partition(self.generation_ops)[2]

    @property
    def generation_comm_f2a_ops(self):
        return self._afd_4stage_partition(self.generation_ops)[3]
    ...
    def __init__(
        ...
        _apply_missing_quant_defaults(self.config)
    ...
    def _is_attn_op(self, op_name: str) -> bool:
        """Return whether an op belongs to the attention/mixer side for AFD."""
        return any(
            marker in op_name
            for marker in (
                "embedding",
    ...
    def _is_ffn_op(self, op_name: str) -> bool:
        """Return whether an op belongs to the FFN/MoE side for AFD."""
        return any(marker in op_name for marker in ("add_norm_2", "ffn", "mlp", "moe", "gate", "logits"))

    @property
    def context_attn_ops(self):
        return [op for op in self.context_ops if self._is_attn_op(op._name)]

    @property
    def context_ffn_ops(self):
        return [op for op in self.context_ops if self._is_ffn_op(op._name)]

    @property
    def generation_attn_ops(self):
        return [op for op in self.generation_ops if self._is_attn_op(op._name)]

    @property
    def generation_ffn_ops(self):
        return [op for op in self.generation_ops if self._is_ffn_op(op._name)]
...
class HybridMoEModel(BaseModel):
    ...
    def _moe_ops(
        ...
        return router_ops + [
            ops.MoEDispatch(
                f"{prefix}_moe_pre_dispatch", count, h, self._topk, self._num_experts, moe_tp, moe_ep, attn_dp, True, enable_afd=self.config.enable_afd
            ),
            ops.MoE(
                f"{prefix}_moe",
                count,
            ...
            ops.MoEDispatch(
                f"{prefix}_moe_post_dispatch", count, h, self._topk, self._num_experts, moe_tp, moe_ep, attn_dp, False, enable_afd=self.config.enable_afd
            ),
        ]
