...
class TaskConfig:
    ...
    def validate(self):
        ...
        def _get_worker_value(wc: object, key: str, default: object = None) -> object:
            if isinstance(wc, Mapping):
                return wc.get(key, default)
            return getattr(wc, key, default)

        def _get_supported_quant_modes(system_name: str, backend_name: str, backend_version: str) -> dict:
            database = get_database(system=system_name, backend=backend_name, version=backend_version)
            return getattr(database, "supported_quant_mode", {}) or {}
            ...
            *,
            ...
            if supported_modes and mode_name not in supported_modes:
                raise ValueError(
                    ...
                    f"backend='{backend_name}', version='{backend_version}'. "
        ...
        def _validate_worker_config(
            ...
            worker_system = str(_get_worker_value(wc, "system_name", self.system_name))
            worker_backend = str(_get_worker_value(wc, "backend_name", self.backend_name))
            worker_version = str(_get_worker_value(wc, "backend_version", self.backend_version))
            try:
                worker_supported = _get_supported_quant_modes(worker_system, worker_backend, worker_version)
            except Exception:
                # If database can't be loaded at all, let downstream handle/report it.
                return

            def _worker_supported_or_raise(op: str, mode_name: str | None) -> None:
                _supported_or_raise(
                    op,
                    mode_name,
                    worker_supported,
                    system_name=worker_system,
                    backend_name=worker_backend,
                    backend_version=worker_version,
                )

            _resolve_model_quant_modes(wc, worker_name)
            _worker_supported_or_raise("gemm", _to_name(_get_cfg_value(wc, "gemm_quant_mode")))

            moe_mode = _to_name(_get_cfg_value(wc, "moe_quant_mode"))
            wc_moe_backend = getattr(wc, "moe_backend", None) or moe_backend
            if worker_backend == "sglang" and wc_moe_backend == "deepep_moe":
                if validate_context:
                    _worker_supported_or_raise("wideep_context_moe", moe_mode)
                if validate_generation:
                    _worker_supported_or_raise("wideep_generation_moe", moe_mode)
            else:
                _worker_supported_or_raise("moe", moe_mode)

            if validate_context:
                _worker_supported_or_raise(context_attn_key, _to_name(_get_cfg_value(wc, "fmha_quant_mode")))

            if validate_generation:
                _worker_supported_or_raise(
                    generation_attn_key,
                    _to_name(_get_cfg_value(wc, "kvcache_quant_mode")),
                )
